...
Для локального теста в браузере оставь:

```env
DEV_TELEGRAM_ID=100001
```

Это демо-пользователь. В production эту строку нужно удалить.
...
В обычном браузере можно открыть `http://localhost:5173` и проверить интерфейс через demo-пользователя.
...
- удалить `DEV_TELEGRAM_ID` из `.env`;
